    ...
    <div className={styles["hamburger_nav"]} style={style}>
      <div className={styles["mypage"]}>
        ...
        <div className={styles["mypage_text"]}>
          {session && nickname ? (
            <Link to="/mypage/userinfo" state={{ session }}>
              마이페이지
            </Link>
          ) : (
      ...
      {session && nickname ? (
        <>
          <div className={styles["user_coin"]}>
            ...
            <button className={styles["userName_btn"]} onClick={handleLogout}>
              로그아웃
            </button>
            <img
          ...
          {userRoom.length > 0 ? (
            userRoom.map((room) => (
              <div
                ...
                onClick={() =>
                  (window.location.href = `/delivery-moa/room/${room.id}`)
                }
              >
                <img
                  className={styles["chat_list_circle"]}
                  src={`${roomUrl}${room.store_id}.jpg`}
                />
                <div className={styles["chat_room"]}>
                  <div className={styles["chat_title_time"]}>
                    ...
                    <div className={styles["chat_time"]}>
                      {room.latest_chat
                        ? new Date(room.latest_chat).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : "대화없음"}
                  ...
                  <div className={styles["chat_room_detail"]}>
                    ...
                    <div className={styles["chat_people"]}>
                      {room.count}/{room.max_people} 참여중
                    </div>
